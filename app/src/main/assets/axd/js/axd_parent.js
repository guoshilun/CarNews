...
function setIFrameParentWindow(){
    window.carnews.toast("准备发送iframe一个window");
    var iframe = document.getElementById('adv_page');
    if (iframe){
        window.carnews.toast("开始发送iframe一个window");
        iframe.contentWindow.setParentWindow(window);
        window.carnews.toast("已发送iframe一个window");
    }else{
        window.carnews.toast("iframe不存在");
    }
};
//通过广告页面的js显示广告
...
function setAdvWidthHeight(width, height){
    <!--window.carnews.toast("设置宽高"+width+"\t"+height);-->
...
function toast(){
    window.carnews.toast("收到iframe回调");
};