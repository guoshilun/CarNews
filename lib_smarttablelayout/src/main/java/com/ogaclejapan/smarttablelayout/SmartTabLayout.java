...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    private static final boolean TAB_LONG_CLICKABLE = true;
    ...
    public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        boolean longClickable = TAB_LONG_CLICKABLE;
        ...
        longClickable = a.getBoolean(
                R.styleable.stl_SmartTabLayout_stl_longClickable, longClickable);
        a.recycle();
        ...
        this.internalTabLongClickListener = longClickable ? new InternalTabLongClickListener() : null;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        tabStrip.onSizeChanged(getMeasuredWidth() / 2);
    }
    ...
    /**
     * Set {@link OnTabLongClickListener} for obtaining long click event.
     *
     * @param listener the {@link OnTabLongClickListener} to set
     */
    ...
    private void scrollToTab(int tabIndex, int positionOffset) {
        ...
        if (tabStrip.isIndicatorOftenInCenter()) {
            int x = selectedTab.getLeft() + positionOffset;
            if (tabIndex > 0 || positionOffset > 0) {
                x -= getWidth() / 2;
                x += tabStrip.getOffsetWhenIndicatorOftenInCenter();
            }
     ...
     * {@link #setCustomTabView(com.ogaclejapan.smarttablelayout.SmartTabLayout.TabProvider)}
     */
            ...
            for (int i = 0; i < tabStrip.getChildCount(); i++) {
                if (v == tabStrip.getChildAt(i)) {
                    if (onTabClickListener != null) {
                        if (onTabClickListener.onTabClicked(i)) {
                            return;
                        }
                    }
