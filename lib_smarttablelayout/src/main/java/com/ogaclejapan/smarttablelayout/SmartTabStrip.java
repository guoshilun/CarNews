 ...
 */
package com.ogaclejapan.smarttablelayout;
...
class SmartTabStrip extends LinearLayout {
    ...
    private static final int DEFAULT_SELECTED_INDICATOR_COLOR = 0xff0159a1;
    ...
    private final boolean indicatorOftenInCenter;
    private final boolean indicatorWithLeftMiddleRight;
    private final int indicatorWithLeftMiddleRightMarginRight;
    ...
    SmartTabStrip(Context context, AttributeSet attrs) {
        ...
        boolean indicatorOftenInCenter = DEFAULT_INDICATOR_IN_CENTER;
        boolean indicatorWithLeftMiddleRight = DEFAULT_INDICATOR_IN_CENTER;
        int indicatorWithLeftMiddleRightMarginRight = 0;
        ...
        indicatorOftenInCenter = a.getBoolean(
                R.styleable.stl_SmartTabLayout_stl_indicatorOftenInCenter, indicatorOftenInCenter);
        indicatorWithLeftMiddleRight = a.getBoolean(
                R.styleable.stl_SmartTabLayout_stl_indicatorWithLeftMiddleRight, indicatorWithLeftMiddleRight);
        ...
        if (indicatorWithLeftMiddleRight) {
            indicatorWithLeftMiddleRightMarginRight = a.getDimensionPixelSize(
                    R.styleable.stl_SmartTabLayout_stl_indicatorWithLeftMiddleRightMarginRight, indicatorWithLeftMiddleRightMarginRight);
        }
        a.recycle();
        ...
        this.indicatorOftenInCenter = indicatorOftenInCenter;
        this.indicatorWithLeftMiddleRight = indicatorWithLeftMiddleRight;
        this.indicatorWithLeftMiddleRightMarginRight = indicatorWithLeftMiddleRightMarginRight;
    ...
    boolean isIndicatorOftenInCenter() {
        return indicatorOftenInCenter;
    }
    ...
    float getSelectionOffset() {
        return selectionOffset;
    }

    float getOffsetWhenIndicatorOftenInCenter() {
        View curTab = getChildAt(selectedPosition);
        ...
        if (selectionOffset > 0f && selectedPosition < getChildCount() - 1) {
            ...
            lineLeft = (selectionOffset * nextTabLeft + (1f - selectionOffset) * lineLeft);
            lineRight = (selectionOffset * nextTabRight + (1f - selectionOffset) * lineRight);
        }

        return (lineRight - lineLeft) / 2;
    }

    void onSizeChanged(int halfWidth) {
        if (indicatorWithLeftMiddleRight) {
            int count = getChildCount();
            if (count > 1) {
                for (int i = 1; i < count; i++) {
                    View curTab = getChildAt(i);
                    View preTab = getChildAt(i - 1);

                    int right = halfWidth - curTab.getMeasuredWidth() - preTab.getMeasuredWidth() / 2 - indicatorWithLeftMiddleRightMarginRight;
                    LayoutParams params = (LayoutParams) preTab.getLayoutParams();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (tabCount > 0) {
            ...
            if (indicatorOftenInCenter) {
                int w = selectedEnd - selectedStart;
                if (w >= indicatorThickness) {
                    int dif = w - indicatorThickness;
                    selectedStart += dif / 2;
                    selectedEnd -= dif / 2;
                }
            }
